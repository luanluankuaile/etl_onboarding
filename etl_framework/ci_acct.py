"""CI_ACCT onboarding processor with version-safe incremental persistence."""
import csv
import hashlib
...
def _clean(row):
    ...
    # Version is the incremental ordering key: null, non-numeric, and negative
    # values are invalid and must never reach the merge candidate set.
...
def run_ci_acct(context, control, pattern="*.csv"):
    ...
    winners = {}
    for row in rows:
        # Within one batch/file, the last row for an equal version wins.
        if row["acct_id"] not in winners or row["version"] >= winners[row["acct_id"]]["version"]:
            winners[row["acct_id"]] = row
    for row in winners.values():
        ...
        existing = persistent.execute("SELECT version, run_id, latest_insert_datetime FROM per_cust_ci_acct WHERE acct_id = ?", (row["acct_id"],)).fetchone()
        # Equal-version records are deliberately rejected. This makes replay
        # deterministic: the first persisted value wins across files/batches.
    ...
    raw.close()
    persistent.close()
    # Replay-safe semantics: manifest is recorded after the persistent commit.
    # A crash in this narrow window can replay the file, but <= version rejection
    # makes replay idempotent and prevents equal-version payload replacement.
    for path in files:
        checksum = hashlib.sha256(path.read_bytes()).hexdigest()
