...
def run_ci_acct(context, control, pattern="*.csv"):
    ...
    for row in winners.values():
        ...
        # Never regress a persistent record: replayed/late data may update only
        # when its version is at least the stored version.  UPDATE preserves the
        # original creation run and insert timestamp; only current audit fields
        # and the payload change.
